...
Creates the 4 identity tables for Module 1 (Auth & Multi-Tenancy):
  companies, org_codes, users, invites

Every tenant-scoped table includes RLS policies in the same migration
(per harness rules). Circular FK between org_codes.created_by and users
is resolved by deferring the org_codes FK constraint until after users
is created.
...
def upgrade() -> None:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 2. org_codes (CompanyBase — company_id only)
    #    NOTE: created_by is a plain UUID column here. The FK to users is
    #    added in step 5 after the users table exists (circular dependency).
    # ──────────────────────────────────────────────────────────────────────
    op.create_table(
        "org_codes",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 3. users (CompanyBase — company_id only). reel48_admin users belong to
    #    an internal "Reel48 Operations" company in the DB; their JWT omits
    #    custom:company_id so the middleware bypasses RLS for cross-company
    #    access.
    # ──────────────────────────────────────────────────────────────────────
    op.create_table(
        "users",
        ...
        sa.CheckConstraint(
            "role IN ('reel48_admin', 'company_admin', 'manager', 'employee')",
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 4. invites (CompanyBase — company_id only)
    # ──────────────────────────────────────────────────────────────────────
    op.create_table(
        "invites",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 5. Deferred FK: org_codes.created_by → users.id
    # ──────────────────────────────────────────────────────────────────────
