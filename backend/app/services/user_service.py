...
VALID_ROLES = {"employee", "manager", "company_admin"}
ADMIN_ASSIGNABLE_ROLES = {"company_admin"}
    ...
    async def create_user(
        ...
        if data.role in ADMIN_ASSIGNABLE_ROLES and context_role not in (
            ...
            raise ForbiddenError(
                f"Only company_admin or above can assign the {data.role} role"
            )
    ...
    async def update_user(
        ...
        if "role" in update_data:
            ...
            if new_role in ADMIN_ASSIGNABLE_ROLES and context_role not in (
                ...
                raise ForbiddenError(
                    f"Only company_admin or above can assign the {new_role} role"
                )
